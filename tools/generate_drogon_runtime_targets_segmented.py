#!/usr/bin/env python3
"""Generate segmented Drogon runtime targets with quantile-aligned source clouds.

This builds on the hybrid reverse-target pass, but replaces single global bbox
alignment with segmented bbox alignment along the dominant axis. The goal is to
improve correspondence for large regions like Body and Wing without touching the
already-proven runtime inversion math.
"""
...
import generate_drogon_runtime_targets_hybrid as hybrid
...
OUT_DIR = OUTPUT / "reverse_drogon_runtime_targets_segmented"
...
MATERIAL_BY_SUBMESH = hybrid.MATERIAL_BY_SUBMESH
UNION_MATERIALS = hybrid.UNION_MATERIALS

SUBMESH_CANDIDATES = {
    "Body_02": ["glb_primitive_bbox", "glb_primitive_seg_z16"],
    "back": ["glb_primitive_bbox", "glb_primitive_seg_z16"],
    "Body": [
        "raw_global",
        "raw_global_seg_z16",
        "glb_primitive_bbox",
        "glb_primitive_seg_z16",
        "glb_body_union_bbox",
        "glb_body_union_seg_z16",
    ],
    "Leg": [
        "raw_global",
        "raw_global_seg_z16",
    ...
    "Wing": [
        ...
        "glb_primitive_seg_z16",
        "glb_wing_union_bbox",
        "glb_wing_union_seg_z16",
    ],
    "Head": ["glb_primitive_bbox", "glb_primitive_seg_z16"],
}
...
def write_combined_obj(
    ...
    with path.open("w", encoding="ascii", newline="\n") as handle:
        handle.write("# combined segmented runtime reverse target preview\n")
        vertex_base = 1
...
def segmented_bbox_align(
    source_points: np.ndarray,
    target_points: np.ndarray,
    axis: int = SEGMENT_AXIS,
    bins: int = SEGMENT_BINS,
) -> np.ndarray:
    source_cuts = np.quantile(source_points[:, axis], np.linspace(0.0, 1.0, bins + 1))
    target_cuts = np.quantile(target_points[:, axis], np.linspace(0.0, 1.0, bins + 1))
    aligned_chunks: list[np.ndarray] = []
    for idx in range(bins):
        if idx < bins - 1:
            source_mask = (source_points[:, axis] >= source_cuts[idx]) & (source_points[:, axis] < source_cuts[idx + 1])
            target_mask = (target_points[:, axis] >= target_cuts[idx]) & (target_points[:, axis] < target_cuts[idx + 1])
        else:
            source_mask = (source_points[:, axis] >= source_cuts[idx]) & (source_points[:, axis] <= source_cuts[idx + 1])
            target_mask = (target_points[:, axis] >= target_cuts[idx]) & (target_points[:, axis] <= target_cuts[idx + 1])

        source_chunk = source_points[source_mask]
        target_chunk = target_points[target_mask]
        if len(source_chunk) == 0 or len(target_chunk) == 0:
            continue
        aligned_chunks.append(hybrid.bbox_align(source_chunk, target_chunk))

    if not aligned_chunks:
        raise ValueError("segmented bbox align produced no chunks")
    return np.vstack(aligned_chunks)
...
def build_candidate_points(
    ...
    primitive_source = hybrid.swizzle_to_world_axes(glb_positions[MATERIAL_BY_SUBMESH[submesh_name]])

    candidates: list[tuple[str, np.ndarray]] = [
        ("glb_primitive_bbox", hybrid.bbox_align(primitive_source, target_points)),
        ("glb_primitive_seg_z16", segmented_bbox_align(primitive_source, target_points)),
    ]

    if submesh_name in {"Body", "Leg", "Wing"}:
        candidates.extend(
            [
                ("raw_global", raw_global),
                ("raw_global_seg_z16", segmented_bbox_align(raw_global, target_points)),
            ]
        )

    if submesh_name == "Body":
        union_source = hybrid.swizzle_to_world_axes(
            np.vstack([glb_positions[name] for name in UNION_MATERIALS["Body_union"] if name in glb_positions])
        )
        candidates.extend(
            [
                ("glb_body_union_bbox", hybrid.bbox_align(union_source, target_points)),
                ("glb_body_union_seg_z16", segmented_bbox_align(union_source, target_points)),
            ]
        )
    elif submesh_name == "Leg":
        union_source = hybrid.swizzle_to_world_axes(
            np.vstack([glb_positions[name] for name in UNION_MATERIALS["Leg_union"] if name in glb_positions])
        )
        candidates.extend(
            [
                ("glb_leg_union_bbox", hybrid.bbox_align(union_source, target_points)),
                ("glb_leg_union_seg_z16", segmented_bbox_align(union_source, target_points)),
            ]
        )
    elif submesh_name == "Wing":
        union_source = hybrid.swizzle_to_world_axes(
            np.vstack([glb_positions[name] for name in UNION_MATERIALS["Wing_union"] if name in glb_positions])
        )
        candidates.extend(
            [
                ("glb_wing_union_bbox", hybrid.bbox_align(union_source, target_points)),
                ("glb_wing_union_seg_z16", segmented_bbox_align(union_source, target_points)),
            ]
        )

    allowed = set(SUBMESH_CANDIDATES[submesh_name])
    return [(name, points) for name, points in candidates if name in allowed]
...
def choose_candidate(
    ...
    for candidate_name, candidate_points in build_candidate_points(submesh_name, target_points, raw_global, glb_positions):
...
def main() -> None:
    ...
    report_lines = [
        "Drogon runtime reverse targets (segmented)",
        "==========================================",
        "",
        "Inputs",
        "------",
        ...
        "Method",
        "------",
        f"- Score segmented source clouds per submesh against the live PIX world positions.",
        f"- Segmentation uses quantile splits on source/target axis {SEGMENT_AXIS} with {SEGMENT_BINS} bins.",
    ...
    for item in submesh_reports:
        ...
        for candidate in item.candidates:
            report_lines.append(
                f"    cand {candidate.name:24s} points={candidate.point_count:6d} rmse={candidate.error.rmse:.9f} avg={candidate.error.avg:.9f}"
            )
